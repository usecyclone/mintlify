...
import { copyToClipboard, CopyToClipboardResult } from './copyToClipboard';
...
export const CodeBlock = forwardRef(function CodeBlock(
  ...
  return (
    <div
      className={clsx(
        'mt-5 mb-8 not-prose bg-[#0F1117] dark:bg-codeblock rounded-xl dark:ring-1 dark:ring-gray-800 relative',
        className
      )}
      ref={ref}
      {...props}
    >
      {filename ? (
        ...
        <Button className="absolute mt-0.5 top-4 right-4" />
      )}
      <div
...
function CodeTabBar({
  ...
  return (
    <div className="flex rounded-t-xl bg-black text-gray-400 text-xs leading-6 border-b border-gray-900 font-medium">
      <div
      ...
      <div className="flex-1 mr-4 flex items-center justify-end">{children}</div>
    </div>
  );
}
...
export function CopyToClipboardButton({
  ...
  return (
    <div className={clsx('group z-10', className || 'relative')}>
      <button className="h-7 w-7 flex items-center justify-center rounded-md" onClick={onCopy}>
        {isCopiedActive ? (
          <svg
            width="16"
            height="11"
      ...
      <div
        className="absolute top-11 left-1/2 transform -translate-x-1/2 -translate-y-1/2 hidden group-hover:block text-white rounded-lg px-1.5 py-0.5 text-xs"
