...
export const useCurrentTableOfContentsSection = (ctx: MDXContentContextType) => {
  ...
  const { tableOfContents, headings, currentTableOfContentsSection } = state;
  ...
  useEffect(() => {
    ...
    function onScroll() {
      ...
      if (currentTableOfContentsSection != current) {
        dispatch({
  ...
  }, [currentTableOfContentsSection, dispatch, headings, tableOfContents]);
